...
for idx, prediction in enumerate(all_prediction):
    for repeat in [prediction]*total_count[idx]:
        repeat_predict.append(repeat)

df = pd.DataFrame(columns=["pred"])
df["pred"] = repeat_predict
df.to_csv("experiment/"+opt.directory+"/submission1.csv")
